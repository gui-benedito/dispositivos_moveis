...
| RF13 | 3      | Alerta de credenciais comprometidas (HIBP)  | Checar vazamentos e alertar usuário (Have I Been Pwned).                  | Alta       |
| RF14 | 3      | Exportação segura                           | Exportar dados em arquivo criptografado com reautenticação.               | Média      |
| RF15 | 3      | Preenchimento automático (Android Autofill) | Integração com Autofill do Android para logins em apps/navegadores.       | Alta       |
| RF16 | 3      | Modo offline                                | App utilizável sem internet; dados locais criptografados; sync ao reconectar. | Alta    |
| RF17 | 3      | Configurações avançadas de segurança        | Ajustar bloqueio, exigir senha mestre, ativar/desativar biometria.        | Média      |
| RF18 | 3      | Notificações inteligentes de segurança      | Alertas de login suspeito, senhas antigas e recomendações.                | Alta       |
| RF19 | 3      | Interface personalizável                    | Temas claro/escuro e ícones por categoria.                                | Média      |

### User Stories por Sprint (renumerado)
